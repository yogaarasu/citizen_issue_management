...
export const LoginPage: React.FC = () => {
  ...
  const [devOtpCode, setDevOtpCode] = useState<string | null>(null); // For development
  ...
  const handleForgotEmailSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
        await sendOtpToEmail(cleanEmail, false); // false = password reset
        setForgotStep(2);
        setSuccess(`OTP sent to ${cleanEmail}.`);
    } catch (e: any) {
        setError(e.message || "Failed to send OTP. Is email registered?");
    } finally {
        setLoading(false);
    }
  };
  ...
  const handleResetSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const cleanEmail = resetEmail.trim().toLowerCase();
      await verifyOtp(cleanEmail, otp.trim(), 'password_reset');
          ...
          <div>
            ...
            {forgotStep === 1 ? (
                ...
                <form onSubmit={handleResetSubmit} className="space-y-4">
                    <p className="text-gray-500 text-sm">Enter code sent to <b>{resetEmail}</b></p>
